...
/**
 * Публичная страница разбора портфолио. Прототип.
 *
 ...
 * Данные подготовленные, ничего не отправляется.
 */
...
type Screen = "form" | "waiting" | "result";
...
const ANALYSIS = {
  ...
  criteria: [
    ["Визуал", "Композиция и типографика на уверенном уровне. Язык не разваливается между экранами."],
    ["UX", "Флоу читаются, паттерны используются по назначению. Пустые состояния проработаны не везде."],
    ["Продуктовое мышление", "Задача обозначена в двух кейсах из трёх, но связь с решениями показана не до конца."],
    ["Исследования", "Упоминаются интервью, но не видно, что именно они изменили в интерфейсе."],
  ],
  strengths: ["Отбор работ сделан — витрины нет", "Служебные состояния показаны", "Язык держится по кейсу"],
  gaps: ["Не хватает «до и после»", "Роль в проектах не указана", "Нет цифр по результату"],
  cases: [
    ["Мобильный банк", "Самый проработанный: показан путь от проблемы до результата."],
    ["Маркетплейс", "Сильный визуально, но задача сформулирована общо."],
    ["Внутренняя панель", "Уместно сдержанный. Визуал здесь не главное, и это правильно."],
  ],
};
  ...
  return (
    <>
      <header className="hero">
        ...
        <p className="subject">Дмитрий Евтеев</p>
        <p className="lede">behance.net/evteev</p>
      </header>

      <Card>
        <p className="pull">{ANALYSIS.overall}</p>
      </Card>

      {/* Заголовок группы стоит на фоне, а не внутри карточки: каждый
          критерий — своя карточка, и накрывать их общей было бы вложением
          карточки в карточку */}
      <h2 className="group">По критериям</h2>
      <Deck items={ANALYSIS.criteria} />

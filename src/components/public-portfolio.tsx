...
/**
 * Предохранитель на длину пояснения. Не рабочий предел, а страховка.
 *
 * Настоящее ограничение стоит в промпте переложения: 200 знаков. Столько
 * укладывается в карточку на обычном телефоне, не растягивая её.
 * Замерено на живой странице настоящим шрифтом: при ширине 390 в карточку
 * входит 218 знаков, при 375 — 202.
 *
 * Здесь предел выше — 260. Смысл разный: обрезка на экране режет фразу и
 * читается как поломка, поэтому пусть лучше карточка немного подрастёт.
 * Распорка это умеет. Обрезаем только совсем уж выходящее за берега:
 * если модель вместо двух строк напишет десять.
 */
const ПРЕДЕЛ_ПОЯСНЕНИЯ = 260;
...
function Deck({ items }: { items: string[][] }) {
  ...
  /**
   * Распорке нужен САМЫЙ ВЫСОКИЙ возможный вид карточки, а он не всегда
   * есть среди настоящих: самый длинный заголовок и самый длинный текст
   * могут быть у разных критериев. Раньше брали одну карточку с самым
   * длинным текстом — и «Продуктовое мышление», переносящееся на две
   * строки, оказывалось выше распорки, а его текст вылезал из колоды.
   * Поэтому собираем составную: длиннейший заголовок плюс длиннейший
   * текст. Такой карточки на экране не будет, но выше неё — ни одной.
   */
  const длиннейшийЗаголовок = items.reduce((a, b) => (b[0].length > a[0].length ? b : a))[0];
  const длиннейшийТекст = items.reduce((a, b) => (b[1].length > a[1].length ? b : a))[1];
    ...
    <div className="deck-wrap">
      ...
      >
        ...
        <div className="card card--full spacer" aria-hidden="true">
          <h3 className="mid">{длиннейшийЗаголовок}</h3>
          <p className="bottom">{длиннейшийТекст}</p>
        </div>
  ...
  return (
    <>
      <header className="hero">
        ...
        <p className="lede">
          Разбираем портфолио с той же логикой, что дизайн-лид агентства, и
  ...
  return (
    <>
      <header className="hero">
        ...
        <p className="lede">
          Обычно это занимает 3–5 минут. Страницу можно закрыть — мы сохраним
          уникальную ссылку, и ты сможешь вернуться к результату в любой
          момент.
        </p>
      </header>
...
function СсылкаНаРазбор({ id }: { id: string }) {
  ...
  return (
    <section className="link-block">
      {/* Пояснения над кнопкой нет намеренно: подзаголовок экрана уже
          говорит про ссылку, и второй текст с ним спорил */}
...
function ResultScreen({ данные }: { данные: ГотовыйРазбор }) {
  ...
  // Без ступени — просто название: «Визуал — null» было бы дико, а
  // «Визуал» с объяснением внутри читается нормально
  const критерии: string[][] = р.критерии.map((k) => [
    k.ступень ? `${k.название} — ${k.ступень}` : k.название,
    обрезатьПоСловам(k.пояснение, ПРЕДЕЛ_ПОЯСНЕНИЯ),
  ]);
  ...
  return (
    <>
      ...
      {р.общее && (
        <Card>
          <h2 className="mid">Общее впечатление</h2>
      ...
      {р.нехватает.length > 0 && (
        <Card>
          <h2 className="mid">Зоны роста</h2>
      ...
      <p className="fineprint">
        Прожарочная собирает разбор автоматически по тому, что видно в
        портфолио — это взгляд на подачу работ, а не оценка тебя как
        специалиста.
      </p>
    </>
  );
  ...
  return (
    <div className={`shell ${lebowski.variable}`}>
      ...
      <style jsx global>{`
        ...
        /*
         * Чёрный уходит на сам документ, а не только на слой страницы.
         *
         * Своей высотой страница закрывает ровно 100dvh. Но в сафари
         * нижняя панель прячется при прокрутке, видимая область
         * становится выше — и снизу выглядывал светлый фон продукта.
         * Он же показывался при оттягивании страницы за край.
         *
         * Стили сняты при уходе с публичной страницы: styled-jsx убирает
         * их вместе с компонентом, и остальному продукту чёрный фон не
         * достаётся.
         */
        html,
        body {
        ...
        /* Оттягивание за край: пусть страница не отходит от краёв вовсе,
           чем показывает из-под себя чужой фон */
        body {
        ...
        /* Текст прижат к низу; 12 — минимальный просвет до заголовка.
           Раньше было 24: на карточке с двухстрочным заголовком этот
           отступ съедал целую строку текста */
        .card--full .bottom { margin-top: auto; padding-top: 12px; }
        ...
        /* Распорка держит высоту колоды по самой длинной карточке.
           ...
           320 — нижняя граница; на узком экране лишняя строка текста

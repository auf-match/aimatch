...
/**
 * Сколько знаков помещается в пояснение критерия.
 *
 * Считано по самому узкому телефону (320): карточка 250 высотой, из них
 * 48 съедают поля, до 68 — заголовок в две строки, 24 — просвет до
 * текста. Остаётся около 110 точек, это четыре строки по 24, а в строку
 * на такой ширине входит около 33 знаков. Отсюда 130 с запасом на
 * перенос длинного слова.
 *
 * Длиннее модель писать умеет, и тогда текст вылезал за карточку.
 */
const ПРЕДЕЛ_ПОЯСНЕНИЯ = 130;
    ...
    <div className={`shell ${lebowski.variable}`}>
      ...
      <style jsx global>{`
        ...
        /* Распорка держит высоту колоды по самой длинной карточке.
           ...
           250 — нижняя граница; на узком экране лишняя строка текста

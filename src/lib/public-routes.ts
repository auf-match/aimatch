 ...
 *
 * /api/public/ — приём заявок с публичной страницы. Лежит под своим
 * префиксом, а не рядом с остальным api, чтобы открытость читалась прямо
 * в адресе: любой маршрут внутри этой папки доступен интернету.
 */
const PUBLIC_PREFIXES = ["/p/", "/api/public/"] as const;

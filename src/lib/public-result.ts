...
export interface PublicCriterion {
  ...
  название: string;
  /**
   * null — оценить не вышло. Не то же самое, что «слабый»: слабый это
   * вывод, а null означает, что выводить было не из чего.
   */
  ступень: Ступень | null;
  пояснение: string;
}
...
/**
 * Что пишем, когда визуал оценить не вышло.
 *
 * Карточка иначе просто исчезала, а пропажа главного критерия читается
 * как поломка — страница ведь называется «Прожарочная».
 *
 * Формулировка намеренно не называет причину. Их две и они разные: в
 * портфолио может не быть изображений вовсе, а может быть так, что все
 * кейсы корпоративные — тогда визуал не оценивается по нашему же
 * правилу, рубрика откалибрована на потребительских интерфейсах. Какая
 * из двух сработала, сюда не доходит, и выдумывать её нельзя: человек с
 * полным картинок портфолио читал бы, что картинок нет.
 */
const БЕЗ_ОЦЕНКИ_ВИЗУАЛА =
  "Визуал оценить не вышло: по тем работам, что открылись, судить о нём " +
  "было не по чему.";
...
export function toPublicResult(разбор: unknown): PublicResult | null {
  ...
  // Разбора нет вовсе — так и говорим. Возврат null для маршрута означает
  // «ещё идёт», и подменять его заглушкой нельзя: страница показала бы
  // готовый результат вместо ожидания
  if (!общее && критерии.length === 0) return null;

  /*
   * Визуал показываем всегда, даже когда оценить его не вышло.
   *
   * Это главный критерий страницы — она и называется «Прожарочная». Его
   * молчаливое исчезновение читается как поломка: человек не понимает,
   * оценили его визуал или нет. Пустоту объясняем словами.
   *
   * Заглушка добавляется только к настоящему разбору, поэтому и стоит
   * ниже проверки на пустоту.
   */
  if (!критерии.some((к) => к.название === "Визуал")) {
    критерии.unshift({ название: "Визуал", ступень: null, пояснение: БЕЗ_ОЦЕНКИ_ВИЗУАЛА });
  }
